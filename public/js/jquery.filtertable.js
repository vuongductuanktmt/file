...
(function($) {
    ...
    $.fn.filterTable = function(options) { // define the filterTable plugin
        ...
        var doFiltering = function(table, q) { // handle the actual table filtering
        		var show_elms;
                var tbody=table.find('.body'); // cache the tbody element
                if (q==='') { // if the filtering query is blank
                    tbody.find('.kpi-row').show().addClass(settings.visibleClass); // show all rows
                    tbody.find('.col').removeClass(settings.highlightClass); // remove the row highlight from all cells
                    if (settings.hideTFootOnFilter) { // show footer if the setting was specified
                        table.find('tfoot').show();
                    }
                } else { // if the filter query is not blank
                    tbody.find('.kpi-row').hide().removeClass(settings.visibleClass); // hide all rows, assuming none were found
                    if (settings.hideTFootOnFilter) { // hide footer if the setting was specified
                        table.find('tfoot').hide();
                    }
                    show_elms=tbody.find('.col').removeClass(settings.highlightClass).filter(':filterTableFind("'+q.replace(/(['"])/g,'\\$1')+'")').addClass(settings.highlightClass).closest('.kpi-row').show().addClass(settings.visibleClass); // highlight (class=alt) only the cells that match the query and show their rows
                ...
                if (settings.callback) { // call the callback function
                  	 //duan add show_elms
                      settings.callback(q, table, show_elms);
                  }
            }; // doFiltering()
            //duan add
           if(typeof(options.doFiltering)=='function'){
        	   doFiltering=options.doFiltering;
           }
        return this.each(function() {
            var t = $(this), // cache the table
                tbody = t.find('.body'), // cache the tbody
            ...
            if (t[0].nodeName==='DIV' && tbody.length>0 
            		&& (settings.minRows===0 || (settings.minRows>0 && tbody.find('.kpi-row').length>settings.minRows)) 
            		&& !t.prev().hasClass(settings.containerClass)) { // only if object is a table and there's a tbody and at least minRows trs and hasn't already had a filter added
