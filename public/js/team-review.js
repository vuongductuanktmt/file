/**
 * Created by quyengeek on 14/04/2017.
 */
/**
...
function initiate_introjs() {
    ...
    $('#KPI').attr("data-intro", "Xem chi tiết đánh giá KPI cá nhân");
    $('#Final').attr("data-step", "2");
    $('#Final').attr("data-intro", "Chọn đánh giá cuối kì hoặc đánh giá tháng");
    $('#Result').attr("data-step", "3");
    $('#Result').attr("data-intro", "Nhập kết quả thực tế");
    $('#savebtn').attr("data-step", "4");
    $('#savebtn').attr("data-intro", "Lưu đánh giá");
    $('#kpistatus').attr("data-step", "5");
    $('#kpistatus').attr("data-intro", "Điều chỉnh trạng thái của KPI. ");
    $('#weighting').attr("data-step", "6");
    $('#weighting').attr("data-intro", "Điều chỉnh trọng số");
}
...
module.controller('PeopleListCtrl', ['$scope', '$http', '$filter', '$sce', '$timeout', function ($scope, $http, $filter, $sce, $timeout) {
    ...
    $scope.get_profile();
    $scope.translate2 = {'most_recent':gettext('most_recent'),'sum':gettext('sum'),'average':gettext('average'),'N/A':gettext('N/A')}
    ...
    $scope.load_kpi = function (id, parent_id) {
        $("#id_parent").html("");
        $("#id_parent").append("<option value=''> --- <i/option>");
        $("#id_name").val("");
        $("#id_description").val("");
    ...
    $scope.delete_kpi = function (user_id, kpi, is_child, parent) {
        if (!confirm("Bạn có muốn xoá KPI \"" + kpi.name + "\" này không?")) {
            return;
        }
        var data = {
            kpi_id: kpi.id,
        ...
        $http.post('/performance/kpi/delete/', data).success(function (data) {
            ...
            $("#waiting").hide();
            alert("Xoá KPI bị lỗi. Hãy thử lại");
        });
    };

    $scope.delete_competency = function (user_id, competency, is_child, parent) {
        if (!confirm("Bạn có muốn xoá KPI \"" + competency.name + "\" này không?")) {
            return;
        }
        var data = {
        ...
        $http.post('/performance/competency/delete/', data).success(function (data) {
            ...
            $("#waiting").hide();
            alert("Xoá năng lực bị lỗi. Hãy thử lại");
        });
    };
    ...
    $scope.assign_kpi_my_team = function (user_id, kpi) {
        if (confirm("KPI \"" + kpi.name + "\" sẽ được phân công xuống cho các thành viên cấp dưới trong nhóm của người này. Bạn có chắc không?")) {
            $http.post('/performance/assign-kpi-for-team-member/', {
                ...
                alert("Phân công KPI không thành công.")
            });
        }
    }
